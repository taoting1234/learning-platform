...
def test_regressor_node(client):
    ...
    for file in files:
        ...
        # 测试模型
        linear_regression(node3)
        k_neighbors_regressor(node3)


def linear_regression(node):
    node.modify(extra={"model": "LinearRegression", "model_kwargs": {}})
    node.run()


def k_neighbors_regressor(node):
    node.modify(
        extra={"model": "KNeighborsRegressor", "model_kwargs": {"n_neighbors": 3}}
    )
    node.run()
    node.modify(
        extra={"model": "KNeighborsRegressor", "model_kwargs": {"n_neighbors": 4}}
    )
    node.run()
    node.modify(
        extra={"model": "KNeighborsRegressor", "model_kwargs": {"n_neighbors": 5}}
    )
    node.run()
